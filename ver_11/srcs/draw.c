...
/*
**	This function deals with window exiting when pressing red cross button
**	on frame. The better place should be inside main function. It has to be
**	moved here because code applying Norm.
*/

int		win_exit(t_combi *c)
{
	(void)c;
	exit(0);
}
...
void	draw_map(t_combi *c)
{
	...
	render_wall(c);
	draw_minimap(c);
	mlx_hook(c->mlx->win_ptr, 17, 0, win_exit, c);
}
